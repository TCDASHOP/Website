(() => {
  ...
  async function setupWorks(site){
    ...
    grid.innerHTML = works.map(w => (
      `<button class="thumb" type="button" data-src="${w.src}" data-title="${escapeHtml(w.title || "")}" aria-label="${escapeHtml(w.title || "")}">
         <img loading="lazy" src="${w.src}" alt="${escapeHtml(w.alt || w.title || "")}">
       </button>`
    )).join("");
  ...
  async function boot(){
    try{
      ...
      applyI18n(site);
      applyConfig(site);
      await setupWorks(site);
    }catch(err){
      console.error(err);
      // fail safe: avoid white screen
