...
const Site = (() => {
  ...
  const applyI18n = (lang) => {
    ...
    document.querySelectorAll("[data-i18n]").forEach((el) => {
      const keyRaw = el.getAttribute("data-i18n") || "";
      const isHtml = keyRaw.startsWith("[html]");
      const key = isHtml ? keyRaw.slice(6) : keyRaw;
...
// Set page title if present
  ...
  const wireDrawer = () => {
    ...
    // Expose for other scripts (page transitions, etc.)
    window.SCADrawer = { open, close };
  };
  ...
  const revealMatrixText = (el, durationMs = 900) => {
    ...
    const step = (now) => {
      ...
      const revealCount = Math.floor(eased * arr.length);

      let out = "";
      for (let i = 0; i < arr.length; i++) {
        const ch = arr[i];
        if (keep(ch) || i < revealCount) out += ch;
  ...
  const wireMatrixReveal = () => {
    if (prefersReducedMotion()) return;

    const markAndReveal = (el) => {
      if (!el) return;
      if (el.dataset.matrixDone === "1") return;

      const text = (el.textContent || "").toString();
      const len = text.length;
      if (!len) return;

      // Avoid heavy work on very long strings (cards can have long bodies)
      const dur = Math.min(1400, Math.max(520, 28 * Math.min(len, 180)));

      el.dataset.matrixDone = "1";
      revealMatrixText(el, dur);
    };

    const isLeafText = (el) => {
      if (!el) return false;
      if (el.childElementCount !== 0) return false; // only leaf nodes
      const text = (el.textContent || "").toString().trim();
      if (!text) return false;
      // ignore icon-only / single glyph / tiny labels
      if (text.length < 3) return false;
      // skip if it looks like an email/url (keep it readable)
      if (/\S+@\S+/.test(text) || /^https?:\/\//i.test(text)) return false;
      return true;
    };

    // Explicit targets
    document.querySelectorAll('[data-matrix="1"]').forEach(markAndReveal);

    // Card text (「カード文まで崩す」)
    document.querySelectorAll('.card').forEach((card) => {
      card
        .querySelectorAll('h1,h2,h3,p,li,a,span,small,em,strong')
        .forEach((el) => {
  ...
  const startTransition = (go) => {
    ...
    if (window.SCADrawer && typeof window.SCADrawer.close === "function") {
      window.SCADrawer.close();
    }
    if (window.MatrixFX && typeof window.MatrixFX.setBoost === "function") {
      window.MatrixFX.setBoost(true);
    }
  ...
  const wirePageTransitions = () => {
    document.addEventListener(
      ...
      (e) => {
        ...
        // Only handle same-site page navigations
        const isHtml =
          url.pathname.endsWith(".html") || url.pathname === "/" || url.pathname === "";
        if (!isHtml) return;

        // If only hash changes, do nothing
        if (
    ...
    window.addEventListener("pageshow", () => {
      ...
      if (window.MatrixFX && typeof window.MatrixFX.setBoost === "function") {
        window.MatrixFX.setBoost(false);
      }
    });
  };
  ...
  const playIntro = () => {
    ...
    document.body.classList.add("isIntro");
    if (window.MatrixFX && typeof window.MatrixFX.setBoost === "function") {
      window.MatrixFX.setBoost(true);
    }

    window.setTimeout(() => {
      document.body.classList.remove("isIntro");
      if (window.MatrixFX && typeof window.MatrixFX.setBoost === "function") {
        window.MatrixFX.setBoost(false);
      }
    }, 650);
  ...
  const init = async () => {
    ...
    // Matrix: page intro + transitions + text reveal
    wirePageTransitions();
    playIntro();
    wireMatrixReveal();
