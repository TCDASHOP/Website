...
(() => {
  ...
  // Cache works.json per page load (WORKS + TIMELINE can both need it)
  let worksCachePromise = null;
  async function loadWorksData(){
    if(worksCachePromise) return worksCachePromise;
    worksCachePromise = (async () => {
      const res = await fetch("/assets/data/works.json", { cache:"no-store" });
      if(!res.ok) throw new Error("works.json not found");
      return await res.json();
    })();
    return worksCachePromise;
  }
  ...
  function applyI18n(dict, lang){
    ...
    // Keep <html lang=".."> synced (accessibility / SEO)
    const htmlLang = (lang === "zh-hans") ? "zh-Hans" : lang;
