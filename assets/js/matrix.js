...
(() => {
  ...
  const MASK_BOOST = 0.78;            // extra alpha inside the mask (0..1)
  ...
  const MASK_Y_ANCHOR = 0.70;         // 0..1 from top (where the phrase center sits)
  ...
  // ====== Ghost pulse (rare clarity spike) ======
  // Creates the “did I just see that?” moment without turning into a billboard.
  const PULSE_MIN = 25;   // seconds
  const PULSE_MAX = 60;   // seconds
  const PULSE_DUR = 0.35; // seconds
  let nextPulseAt = 0;
  let pulseUntil = 0;
  ...
  function tick(now) {
    ...
    // Ghost pulse: after the story is fully revealed, occasionally raise clarity briefly.
    let pulse = 0;
    if (!REDUCE_MOTION && storyA2 > 0.95) {
      if (!nextPulseAt) nextPulseAt = t + rand(PULSE_MIN, PULSE_MAX);
      if (t >= nextPulseAt) {
        pulseUntil = t + PULSE_DUR;
        nextPulseAt = t + rand(PULSE_MIN, PULSE_MAX);
      }
      if (t < pulseUntil) {
        const mid = pulseUntil - PULSE_DUR * 0.5;
        const x = 1 - Math.abs((t - mid) / (PULSE_DUR * 0.5));
        pulse = clamp01(x);
      }
    }
    // fade previous frame
    ...
    for (let i = 0; i < cols.length; i++) {
      ...
      for (let s = 0; s < steps; s++) {
        ...
        const mv2 = mv * (1 + 0.65 * pulse);
        const alpha = BASE_ALPHA + (inMask ? (MASK_BOOST * mv2) : 0);
        const glow = inMask ? (GLOW_BOOST * mv2) : 0;
        ...
        const sat2 = clamp01(c.sat + (inMask ? (0.18 + 0.10 * pulse) : 0.00));
        const lit2 = clamp01(c.lit + (inMask ? (0.18 + 0.10 * pulse) : 0.00) + 0.10 * glow);
