(function(){
  ...
  function init(){
    ...
    let last = 0;
    let running = true;
  // Mobile tuning: phones have less GPU/thermal headroom; keep the vibe but cut the workload.
  ...
  const maxDPR = isMobile ? 1.5 : 2;
    ...
    // On mobile, pause while the user is scrolling/touching to avoid jank.
    if (isMobile) {
      let _t;
      const _pause = () => {
        running = false;
        clearTimeout(_t);
        _t = setTimeout(() => { running = true; last = performance.now(); }, 220);
      };
      window.addEventListener('scroll', _pause, { passive: true });
      window.addEventListener('touchmove', _pause, { passive: true });
      window.addEventListener('touchstart', _pause, { passive: true });
    }
    ...
    function frame(ts){
      ...
      if (ts - last < minFrame) { requestAnimationFrame(frame); return; }
      last = ts;
      // Fade (trail)
